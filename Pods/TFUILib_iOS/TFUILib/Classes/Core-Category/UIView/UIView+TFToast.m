...
static const CGFloat TFToastHorizontalPadding   = 10.0;
...
- (void)makeToast:(NSString *)message {
    [self makeToast:message duration:TFToastDefaultDuration position:nil];
}
...
- (void)showToast:(UIView *)toast {
    [self showToast:toast duration:TFToastDefaultDuration position:nil];
}
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:TFToastPositionTop] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + 64);
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - TFToastVerticalPadding);
}
