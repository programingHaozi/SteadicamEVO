...
@property (nonatomic,copy) TFProgressHudBlock block;
...
{
    TFProgressHud *hud = [[TFProgressHud alloc] initWithView:view];
...
+ (void)hideAllHudInView:(UIView *)view animated:(HudAnimatedType)animatedType
{
    ...
    {
        if([viewTemp isKindOfClass:[TFProgressHud class]])
        {
            TFProgressHud *hud = (TFProgressHud *)viewTemp;
...
#pragma mark - 结果视图(在设置的时间过后会消失) 绘图：带文字，不消失
